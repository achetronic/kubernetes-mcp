...
//   - undo_rollout only accepts apps/deployments.
package k8stools
...
// Regression: undo_rollout only supports apps/deployments. The previous code
...
func TestE2E_UndoRollout_RejectsNonAppsGroup(t *testing.T) {
	...
	requireContains(t, text, "apps/deployments", "expected apps/deployments-only message")
}

func TestE2E_UndoRollout_RejectsNonDeploymentResource(t *testing.T) {
	e := newE2EEnv(t)
	...
	text := expectErr(t, res, "undo_rollout for statefulsets should be rejected")
	requireContains(t, text, "apps/deployments", "expected apps/deployments-only message")
}
